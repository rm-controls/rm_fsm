...
class StateStandby : public StateBase {
 ...
 protected:
  void setChassis() override {
    ...
    if (data_->pos_x_ >= move_distance_ * 0.5 - stop_distance_) vel_2d_cmd_sender_->setLinearXVel(-1.);
    else if (data_->pos_x_ <= move_distance_ * 0.1 + stop_distance_) vel_2d_cmd_sender_->setLinearXVel(1.);
  }
  void setGimbal(SideCommandSender *side_cmd_sender) override {
    if (side_cmd_sender->pos_yaw_ >= side_cmd_sender->yaw_min_ * 0.1) side_cmd_sender->yaw_direct_ = -1.;

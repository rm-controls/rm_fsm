...
FsmStandard<T>::FsmStandard(ros::NodeHandle &node_handle) : Fsm<T>(node_handle) {
  ...
  state_fly_slope_ = new StateFlySlope<T>(&this->data_, "flyslope", node_handle, this->pc_control_);
  state_burst_ = new StateBurst<T>(&this->data_, "burst", node_handle, this->pc_control_);
